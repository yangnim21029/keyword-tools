...
import { generateFinalPromptStep } from '@/app/actions/actions-ai-writing';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validation.success) {
      console.error(
        '[API /writing/7-generate-final-prompt] Invalid input:',
    ...
    console.log(
      `[API /writing/7-generate-final-prompt] Calling action step for Keyword: ${inputData.keyword}`
    );
    ...
    console.log(
      `[API /writing/7-generate-final-prompt] Action step complete for Keyword: ${inputData.keyword}`
    );
    // Return the result from the action step { finalPrompt: string }
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error(
      '[API /writing/7-generate-final-prompt] Error calling action step:',
      error
    );
    const errorMessage =
    ...
    return NextResponse.json(
      {
        error: 'Failed during final prompt generation step',
