...
import { GscData } from "../gsc-action";
...
export function RawTable({
  ...
  const [rawData, setRawData] = useState<GscData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const { sortedRawData } = await getThemeData(theme);
        setRawData(sortedRawData);
      } catch (error) {
      ...
      renderRow={(item, index) => (
        ...
        >
          ...
          <td className="px-4 py-1.5 text-right">
            <span
              className={`px-1.5 py-0.5 rounded font-mono ${
                item.mean_position <= 3
                  ? "bg-green-100 text-green-800"
                  : item.mean_position <= 10
                    ? "bg-blue-100 text-blue-800"
                    : item.mean_position <= 20
          ...
          <td className="px-4 py-1.5 text-right font-mono text-gray-700">
            {item.min_position.toFixed(1)}
          </td>
          <td className="px-4 py-1.5 text-right font-mono text-gray-700">
            {item.max_position.toFixed(1)}
          </td>
          <td className="px-4 py-1.5 text-right font-mono text-gray-700">
          ...
          <td className="px-4 py-1.5 text-right font-mono text-gray-700">
            {item.overall_ctr.toFixed(2)}%
          </td>
          <td className="px-4 py-1.5 text-xs max-w-[250px]">
            {Array.isArray(item.associated_pages) &&
            item.associated_pages.length > 0 ? (
