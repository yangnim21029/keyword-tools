...
export async function fetchGscData(
  ...
  const response = await fetch(
    "https://gsc-weekly-analyzer-241331030537.asia-east2.run.app/analyze/all",
    {
      next: {
...
export async function processPageData(uniqueDataItems: GscData[]) {
  const aggregatedStatsMap = new Map<
    string,
    Map<
      string,
      {
  ...
  for (const item of uniqueDataItems) {
    ...
    siteTotalImpressions.set(
      normalizedSiteId,
      currentTotal + item.total_impressions,
    );
  }
  ...
  for (const uniqueItem of uniqueDataItems) {
    ...
    if (
      !Array.isArray(uniqueItem.associated_pages) ||
      uniqueItem.associated_pages.length === 0
    )
      continue;
    ...
    const representativeCleanUrl = await getCleanUrl(
      uniqueItem.associated_pages[0],
    );
  ...
  for (const [normalizedSiteId, pages] of aggregatedStatsMap.entries()) {
    ...
    const sortedKeywords = Array.from(stats.keywords).sort((a, b) => {
      ...
      const impDiff =
        (bData?.total_impressions ?? 0) - (aData?.total_impressions ?? 0);
...
export async function processSiteKeywordData(
  uniqueDataItems: GscData[],
): Promise<SiteKeywordData[]> {
  const siteKeywordMap = new Map<
    string,
    {
  ...
  for (const item of uniqueDataItems) {
    ...
    const keywordData = siteData.keywordStats.get(item.keyword) || {
      impressions: 0,
      clicks: 0,
    };
  ...
  for (const [siteId, siteData] of siteKeywordMap.entries()) {
    const sortedKeywords: KeywordPerformance[] = Array.from(
      siteData.keywordStats.entries(),
    )
      .map(([keyword, stats]) => ({
        keyword,
    ...
    gscData: [
      {
        ...
        overall_ctr: 100.0,
        associated_pages: [
          "https://girlstyle.com/tw/article/watsons-oliveyoung年度最好用面膜top-10-第4款赵露思推荐-养成婴儿肌必备",
        ],
      },
      {
        site_id: "girlstyle_com",
        ...
        overall_ctr: 100.0,
        associated_pages: [
          "https://girlstyle.com/tw/article/防曬spf-化妝水-保養常識9成都是騙人的-落合博子",
        ],
      },
      {
        ...
        overall_ctr: 0.0,
        associated_pages: [
          "https://girlstyle.com/my/article/微针精华-最全攻略-功效-使用方法-产品推荐",
        ],
      },
      {
        ...
        overall_ctr: 0.38,
        associated_pages: [
          "https://topbeautyhk.com/article/精華液-美白-保濕-抗衰老-推介",
        ],
      },
      {
      ...
      },
      {
        ...
        overall_ctr: 1.3,
        associated_pages: [
          "https://pretty.presslogic.com/article/色斑-淡斑精華",
        ],
      },
    ],
    pageData: [
...
export async function generateAiAnalysis(
  ...
  const keywordDataValue = sortedData
    .map((item) => {
...
`;
  ...
  console.log(
    `[Action] Calling AI for GSC Analysis TEXT using ${model} with ${itemCount} raw items...`,
  );
  try {
    ...
    const validatedResult = GscAnalysisTextSchema.parse({
...
export async function triggerAiAnalysisForTheme(
  theme: string,
): Promise<GscAnalysisText> {
  "use server";
  ...
  try {
    ...
    console.log(
      `[Action] AI Analysis generated successfully for theme: ${theme}`,
    );
    return analysisResult;
  } catch (error) {
    console.error(
      `[Action] Failed to trigger AI analysis for theme ${theme}:`,
      error,
    );
