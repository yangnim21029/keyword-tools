...
const StepChecklist = ({ steps }: { steps: Step[] }) => {
    const getIcon = (status: Step['status']) => {
        switch (status) {
            ...
            case 'pending':
            default: return <Circle className="h-4 w-4 text-gray-300 dark:text-gray-600" />;
        }
    };
...
export const ProgressChecklistDisplay: React.FC<ProgressChecklistDisplayProps> = ({ steps }) => {
    return (
