...
import {
  Check, ChevronsUpDown, Layers, Loader2, TerminalSquare
} from "lucide-react";
import type { KeywordVolumeListItem, KeywordVolumeObject } from "@/app/services/firebase/schema";
...
interface PromptGeneratorFormProps {
  ...
  hasClusters: boolean;
  handleSubmitPrompt: (event?: React.FormEvent<HTMLFormElement>) => Promise<void>;
}
  ...
  return (
    <form onSubmit={handleSubmitPrompt} className="space-y-6">
      ...
      <div className="space-y-2">
        <Label htmlFor="keyword-combobox" className="text-base font-medium">Keyword</Label>
        <div className="relative">
          <Popover open={comboboxOpen} onOpenChange={setComboboxOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                disabled={isLoadingPrompt || isListLoading || isDetailLoading || isGeneratingArticle}
            ...
            <PopoverContent align="start" className="p-0 w-[--radix-popover-trigger-width]">
              <Command shouldFilter={false} className="p-0">
                ...
                />
                <CommandList>
                  {isListLoading && <div className="p-4 text-center text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin mr-2 inline-block" />Loading...</div>}
                  ...
                  {!isListLoading && !listFetchError && realKeywordList.length === 0 && <CommandEmpty>No keywords found.</CommandEmpty>}
                        ...
                        <CommandItem
                          ...
                          onSelect={async (currentValue: string) => {
                            ...
                            try {
                              ...
                              setSelectedKeywordReport(detailResult as KeywordVolumeObject | null);
                              if (!detailResult) toast.error("Could not fetch keyword details.");
                            } catch (error) { toast.error(error instanceof Error ? `Error fetching details: ${error.message}` : "Unknown error fetching details."); }
          ...
          <div className="absolute right-2 top-2 h-8 flex items-center">
             ...
             <Button
                 ...
                 disabled={isLoadingPrompt || isDetailLoading || isGeneratingArticle || !keyword || !mediaSiteName}
                 className={cn(
                 ...
                 (isLoadingPrompt || isDetailLoading || isGeneratingArticle) && "opacity-50 cursor-not-allowed",
                 )}
             >
                 {isLoadingPrompt ? (
        ...
        {!showMediaSiteOptions && (
          isMounted && mediaSiteName ? (
            (() => {
              ...
              let hostname = ""; try { hostname = new URL(site?.url || ".").hostname; } catch (e) { /* ignore */ }
              ...
              return (
                <Button
                  ...
                  className={cn("w-full justify-start text-left font-normal h-10 text-sm")}
                >
                  {faviconUrl && <Image src={faviconUrl} alt="" width={16} height={16} className="w-4 h-4 mr-2 flex-shrink-0" unoptimized />}
                  <span className="truncate">Media Site: {mediaSiteName}</span>
                </Button>
              );
            })()
          ) : (
            <Button type="button" variant="outline" size="sm" onClick={() => setShowMediaSiteOptions(true)} disabled={isLoadingPrompt || isDetailLoading || isGeneratingArticle} className="w-full justify-start text-left font-normal h-10 text-sm text-muted-foreground">
              Select Media Site...
            </Button>
          )
        )}
        ...
        <div className="border border-gray-300 dark:border-neutral-700 p-3 space-y-2 bg-white dark:bg-neutral-900 rounded-md mt-1">
          ...
          <div className="flex flex-wrap gap-2">
            {MEDIASITE_DATA.map((site) => {
              ...
              return (
                <Button key={site.name} type="button" variant="outline" size="sm"
                  onClick={() => { setMediaSiteName(site.name); setShowMediaSiteOptions(false); }}
                  disabled={isLoadingPrompt || isDetailLoading || isGeneratingArticle}
                  className={cn("flex items-center gap-2 px-3 py-1.5 text-xs font-mono", mediaSiteName === site.name && "ring-2 ring-primary ring-offset-background")}
                >
      ...
      {showFineTuneOptions && (
        <div className="border border-gray-300 dark:border-neutral-700 p-3 space-y-3 bg-white dark:bg-neutral-900 rounded-md mt-1">
          <p className="text-xs font-mono text-gray-500 dark:text-gray-400">SELECT_FINE_TUNE_SETS:</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {allFineTuneNames.map((name) => (
              <div key={name} className="flex items-center space-x-2">
                <Checkbox id={`fine-tune-${name}`} checked={selectedFineTunes.includes(name)}
                  onCheckedChange={(checked) => handleFineTuneChange(checked, name)}
                  disabled={isLoadingPrompt || isDetailLoading || isGeneratingArticle} />
                <Label htmlFor={`fine-tune-${name}`} className="text-sm font-mono text-gray-700 dark:text-gray-300 cursor-pointer">{name}</Label>
              </div>
            ))}
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Selected sets adjust the final prompt.</p>
        </div>
      )}
      ...
      {hasClusters && (
        <div className="space-y-2">
          <Label htmlFor="cluster-select" className="text-base font-medium">Target Cluster / Persona (Optional)</Label>
          <Select value={selectedClusterName} onValueChange={setSelectedClusterName} disabled={isLoadingPrompt || isDetailLoading || isGeneratingArticle}>
            <SelectTrigger id="cluster-select" className="w-full h-10 text-sm bg-gray-50 dark:bg-neutral-900 border-gray-300 dark:border-neutral-700 focus-visible:ring-primary hover:bg-gray-100 dark:hover:bg-neutral-800">
          ...
          {selectedClusterName !== "__ALL_CLUSTERS__" && (
            <div className="mt-2 p-3 border border-dashed border-indigo-300 dark:border-indigo-700 rounded-md bg-indigo-50/50 dark:bg-indigo-900/10 text-sm text-indigo-800 dark:text-indigo-200">
              {displayedPersona ? (
                <><p className="font-medium mb-1">Persona: {selectedClusterName}</p><p className="text-xs opacity-80 line-clamp-3">{displayedPersona}</p></>
              ) : (
                <p className="text-xs opacity-70 italic">(Persona description loading or not found)</p>
              )}
            </div>
          )}
        </div>
