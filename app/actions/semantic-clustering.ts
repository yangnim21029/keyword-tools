 ...
 * @returns 返回聚类结果或抛出错误
 */
export async function performSemanticClusteringAI(input: { keywords: string[], model?: 'gpt-4o' | 'gpt-4o-mini' }) {
  try {
    // 验证输入
 ...
 */
async function _prepareKeywordsForClustering(researchId: string): Promise<{ success: boolean; keywords: string[] | null; error?: string }> {
    if (!db) {
        console.error('[Clustering Prep] Database not initialized');
        return { success: false, keywords: null, error: 'Database not initialized' };
    }
    if (!researchId) {
    ...
    try {
        ...
        if (!docSnap.exists) {
            ...
            return { success: false, keywords: null, error: 'Research item not found.' };
        }
        ...
        console.log(`[Clustering Prep] Found ${keywordTexts.length} keywords for ${researchId}.`);
        return { success: true, keywords: keywordTexts };

    } catch (error) {
        console.error(`[Clustering Prep] Failed for ${researchId}:`, error);
        const message = error instanceof Error ? error.message : 'Keyword preparation failed.';
 ...
 * @private Internal helper function.
 */
async function _saveClusteringResults(researchId: string, clusters: Record<string, string[]>): Promise<{ success: boolean; error?: string }> {
    try {
        if (Object.keys(clusters).length === 0) {
            console.log(`[Clustering Save] No clusters generated by AI for ${researchId}. Skipping save.`);
        ...
        console.log(`[Clustering Save] Saving ${Object.keys(clusters).length} clusters for ${researchId}`);
        ...
        const message = error instanceof Error ? error.message : 'Saving clustering results failed.';
 ...
 * @private Internal helper function.
 */
async function _attemptRevalidationOnError(researchId: string, context: string): Promise<void> {
    try {
        console.warn(`[Clustering Action] Attempting cache revalidation for ${researchId} after ${context}.`);
        await revalidateKeywordResearchCache(researchId);
    } catch (revalError) {
        // Log critical failure but don't let it stop the main error flow
        console.error(`[Clustering Action] CRITICAL: Failed to revalidate cache for ${researchId} after ${context}:`, revalError);
    }
}
...
export async function requestClustering(
  ...
  try {
      ...
      if (!prepResult.keywords || prepResult.keywords.length === 0) {
          console.log(`[Clustering Action] No keywords found for ${researchId}. Process finished.`);
      ...
      try {
          ...
          clusteringResult = await performSemanticClusteringAI({ keywords: prepResult.keywords });
      } catch (aiError) {
          console.error(`[Clustering Action] AI Clustering failed for ${researchId}:`, aiError);
          await _attemptRevalidationOnError(researchId, 'AI failure'); // Attempt revalidation on AI error
          const message = aiError instanceof Error ? aiError.message : 'AI clustering failed.';
          return { success: false, error: message };
      }

      // 3. Save Results & Revalidate Cache
      const saveResult = await _saveClusteringResults(researchId, clusteringResult.clusters);
      ...
      // 4. Final Success Log
      console.log(`[Clustering Action] Clustering completed successfully for ${researchId}.`);
      return { success: true };
