...
export async function submitCreateScrape({
  ...
  try {
    ...
    if (!scrapedContent) {
      console.error(
        `[Action: Create Scrape] Failed to extract content from ${url}.`,
      );
      return {
        success: false,
    ...
    if (!newDocId) {
      console.error(
        `[Action: Create Scrape] Failed to save scraped content for ${url} to Firestore.`,
      );
      return { success: false, error: "Failed to save scraped content." };
    }

    console.log(
      `[Action: Create Scrape] Successfully scraped and saved content for ${url}. New ID: ${newDocId}`,
    );
    ...
    console.error(
      `[Action: Create Scrape] Unexpected error processing ${url}:`,
      error,
    );
    return {
      success: false,
