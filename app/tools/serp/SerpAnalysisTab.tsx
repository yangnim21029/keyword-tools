...
const renderEmptyState = () => (
  <EmptyState
    title="尚未進行 SERP 分析"
    description="輸入關鍵詞並點擊分析按鈕，分析搜索引擎結果頁面，了解競爭情況。"
    icon={<LayoutGrid className="h-12 w-12 text-gray-300 dark:text-gray-700" />}
    actionLabel="了解更多 SERP 分析工具"
    onAction={() => window.open("https://example.com/serp-analysis-guide", "_blank")}
  />
)
...
const renderLoadingState = (message: string) => (
  <Card className="p-8 flex flex-col items-center justify-center">
    <div className="h-10 w-10 border-t-2 border-b-2 border-purple-500 rounded-full animate-spin mb-4"></div>
...
export default function SerpAnalysisTab({
  ...
  }, [serpKeywords, region, language, setGlobalLoading])
  ...
  const handleSerpHtmlAnalysis = async () => {
    ...
    try {
      ...
      if (errorMessage) {
        ...
        if (updatedResults.sourceInfo) {
          toast.info(updatedResults.sourceInfo)
        }
        if (updatedResults.error) {
          toast.error(updatedResults.error)
          throw new Error(updatedResults.error)
        }
  ...
  return (
    <div className="space-y-6">
      {error && (
        <div
          className="bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-xl relative mb-4"
          role="alert"
        >
      ...
      {isLoading ? (
        renderLoadingState(loadingMessage || "分析 SERP 結果中...")
      ) : (
        <>
          {/* If no results and not loading, show empty state */}
          {(!serpResults || !Object.keys(serpResults.results || {}).length) && !isLoading && renderEmptyState()}
