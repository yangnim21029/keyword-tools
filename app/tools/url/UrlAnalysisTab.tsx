...
import { fetchSearchVolume, getUrlSuggestions, saveKeywordResearch, updateHistoryWithClusters } from "@/app/actions"
import type { SearchVolumeResult, SuggestionsResult } from "@/app/types"
...
import { Globe, LayoutGrid, Save } from "lucide-react"
...
// Add a new empty state component
const renderEmptyState = () => (
  <EmptyState
    title="尚未進行 URL 分析"
    description="輸入網址並點擊分析按鈕，從網頁中提取關鍵詞並獲取搜索量數據。"
    icon={<Globe className="h-12 w-12 text-gray-300 dark:text-gray-700" />}
    actionLabel="了解更多 URL 分析工具"
    onAction={() => window.open("https://example.com/url-analysis-guide", "_blank")}
  />
)
...
const renderLoadingState = (message: string) => (
  <Card className="p-8 flex flex-col items-center justify-center">
    <div className="h-10 w-10 border-t-2 border-b-2 border-green-500 rounded-full animate-spin mb-4"></div>
...
export default function UrlAnalysisTab({
  ...
  const [volumeData, setVolumeData] = useState<SearchVolumeResult>({
    results: [],
  ...
  // Optimize the handleGetUrlSuggestions function with useCallback
  const handleGetUrlSuggestions = useCallback(async () => {
    if (!url.trim()) {
      return
    }
  ...
  }, [url, region, language, setGlobalLoading])
          ...
          if (url) {
            try {
              ...
              if (selectedHistoryDetail && selectedHistoryDetail.id) {
                ...
                const result = await updateHistoryWithClusters(selectedHistoryDetail.id, finalJsonResult.clusters)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 需要一個隱藏或標識的按鈕供全局點擊 */}
      <LoadingButton
        id="url-analysis-submit"
        onClick={handleGetUrlSuggestions}
        style={{ display: "none" }}
        isLoading={isLoading}
      >
        Submit
      ...
      {isLoading ? (
        renderLoadingState(loadingMessage || "分析URL中...")
      ) : (
        <>
          {error && (
          ...
          {!suggestions.suggestions.length && !volumeData.results.length && !isLoading && renderEmptyState()}
              ...
              {step === "clusters" && clusters && (
                <Button
                  onClick={() => {
                    if (selectedHistoryDetail && selectedHistoryDetail.id) {
                      console.log("準備更新分群結果到歷史記錄 ID:", selectedHistoryDetail.id)
                      updateHistoryWithClusters(selectedHistoryDetail.id, clusters).then(
                        (result: { success: boolean; error?: string }) => {
                          if (result.success) {
                            console.log("已更新歷史記錄的分群結果")
                            toast.success(`已成功更新分群結果到歷史記錄！`)
                          } else {
                            console.error("更新分群結果失敗:", result.error)
                            toast.error(`更新分群結果失敗: ${result.error}`)
                          }
                        },
                      )
                    } else {
              ...
              <Card className="p-4 border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-950 shadow-sm rounded-xl">
                ...
                <div className="overflow-y-auto h-[500px]">
                  {clusters.map((cluster: any, clusterIndex: number) => (
                    <div
                      key={clusterIndex}
                      className="mb-4 p-2 border border-gray-100 dark:border-gray-800 rounded-lg bg-gray-50 dark:bg-gray-900/50"
                    >
                    ...
                    <TableBody>
                      {volumeData.results
                        ...
                        .map((result, i) => (
                          <TableRow
                            key={i}
                            className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-900/30"
                          >
