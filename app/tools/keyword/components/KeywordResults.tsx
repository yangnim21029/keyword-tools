...
import { KeywordVolumeResult } from '@/lib/schema';
import { ArrowUpDown, BarChart2, SortAsc, SortDesc } from 'lucide-react';
...
export default function KeywordResults({
  ...
  const { sortedItems, groupedKeywords } = useMemo(() => {
    ...
    // 按搜索量分组，简单分为高、中、低三组
    const highVolume = sorted
      .filter(item => (item.searchVolume || 0) > 1000)
      .slice(0, ITEMS_PER_GROUP);
    const mediumVolume = sorted
      .filter(
        item =>
          (item.searchVolume || 0) > 100 && (item.searchVolume || 0) <= 1000
      )
      .slice(0, ITEMS_PER_GROUP);
    const lowVolume = sorted
      .filter(item => (item.searchVolume || 0) <= 100)
      .slice(0, ITEMS_PER_GROUP);
  ...
  return (
    <div className="space-y-2" ref={containerRef}>
      <div className="flex items-center justify-between">
        ...
        >
          {currentSort.icon}
          <span className="ml-1 text-xs sm:inline hidden">
            {currentSort.label}
          </span>
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {groupedKeywords.slice(0, visibleGroups).map((group, groupIndex) => (
          <div
            ...
            className="bg-gray-50 dark:bg-gray-800/40 rounded-lg p-3"
          >
