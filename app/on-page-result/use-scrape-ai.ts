...
export async function generateQuestionsFromReference(
    ...
    console.log(`[AI Analysis] Starting outline generation for keyword: "${keyword}"`);
...
`;
        ...
        console.error("[AI Analysis] Error during outline generation pipeline:", error);
        throw new Error(`AI analysis pipeline failed: ${error instanceof Error ? error.message : String(error)}`);
    }
}
