 ...
 */
export async function fetchOutlineHeadings({
  urls,
}: {
  urls: string[];
}): Promise<string> {
  ...
  console.log(
    `[fetchOutlineHeadings] Starting fetch for max ${maxFetches} URLs from ${urls.length} provided.`,
  );

  for (const url of urls) {
    if (successfulFetches >= maxFetches) {
      console.log(
        `[fetchOutlineHeadings] Reached max fetch limit (${maxFetches}). Stopping.`,
      );
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: {
          // Add a basic User-Agent to mimic a browser
          "User-Agent":
      ...
      if (!response.ok) {
        console.warn(
          `[fetchOutlineHeadings] Failed to fetch ${url}: ${response.status} ${response.statusText}`,
        );
      ...
      if (typeof cheerio?.load !== "function") {
        console.error(
          "[fetchOutlineHeadings] Cheerio library is not loaded correctly. Please install 'cheerio'.",
        );
      ...
      if (headingsFromThisUrl) {
        ...
        console.log(
          `[fetchOutlineHeadings] Successfully processed ${url} (${successfulFetches}/${maxFetches}).`,
        );
      } else {
        console.log(
          `[fetchOutlineHeadings] No h2/h3 headings found at ${url}.`,
        );
      }
    } catch (error: any) {
      ...
      if (error.name === "TimeoutError") {
        console.error(
          `[fetchOutlineHeadings] Timeout fetching ${url}: ${error.message}`,
        );
      } else {
        console.error(
          `[fetchOutlineHeadings] Error processing ${url}: ${error.message}`,
        );
      }
  ...
  console.log(
    `[fetchOutlineHeadings] Finished fetching. Processed ${successfulFetches} URLs.`,
  );
