  ...
  try {
    ...
    if (
      ...
      if (suggestions.length === relevanceScores.length) {
        ...
        console.log(`[Service: fetchAutocomplete] Filtered Suggestions (>=600):`, filteredSuggestions);
        return filteredSuggestions;
      } else {
         console.warn('[Service: fetchAutocomplete] Suggestions and scores length mismatch.');
        ...
        console.warn('[Service: fetchAutocomplete] Relevance data not found, returning all suggestions.');
        return data[1];
    }
...
export async function fetchSuggestionWithDelay(
  ...
  try {
    ...
    console.log(`[Service: fetchSuggestionWithDelay] Query: ${query}, Region: ${region}, Language: ${language}`);
    ...
    if (
      ...
      if (suggestions.length === relevanceScores.length) {
        ...
        console.log(`[Service: fetchSuggestionWithDelay] Filtered Suggestions (>=600):`, filteredSuggestions);
        return filteredSuggestions;
      } else {
          console.warn('[Service: fetchSuggestionWithDelay] Suggestions and scores length mismatch.');
          return suggestions; // 返回所有建議
      }
    } else if (Array.isArray(data) && data.length > 1 && Array.isArray(data[1])) {
        console.warn('[Service: fetchSuggestionWithDelay] Relevance data not found, returning all suggestions.');
        return data[1]; // 返回所有建議 (舊的邏輯)
    }

    console.log('[Service: fetchSuggestionWithDelay] No valid suggestions found.');
    return [];
  } catch (error) {
