...
export async function generateRelatedKeywordsAI(
  ...
  // --- Updated Prompt ---
  const prompt = `你是一位專業的關鍵詞研究專家，專注於 ${region} 地區、使用 ${language} 語言的市場。
針對以下核心查詢：「${query}」

請生成大約 ${count} 個與其高度相關的補充關鍵詞建議。這些建議應該包含：
...
3.  **具體補充詞**：
    ...
    *   潛在的長尾關鍵詞組合（例如 "香奈兒手錶推薦款", "如何保養香奈兒手錶"）。
4.  目標是混合生成不同層次的關鍵字，找出較有可能具有搜尋量、且能擴展原始查詢涵蓋範圍的字詞。

請將生成的關鍵詞建議以 JSON 數組的格式返回，例如：["香奈兒", "精品手錶", "香奈兒手錶價格", ...]
