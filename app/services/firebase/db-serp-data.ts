...
import type { FirebaseSerpDataDoc } from './schema'; // Import the specific type

// Helper to convert Firestore Timestamps in fetched data
function convertSerpDataTimestamps<T extends FirebaseSerpDataDoc>(data: T): Omit<T, 'createdAt' | 'updatedAt'> & { createdAt?: Date; updatedAt?: Date } {
...
/**
 * Saves a new SERP Data document to Firestore.
 * @param dataToSave Object conforming to FirebaseSerpDataDoc, but without id, createdAt, updatedAt.
 * @returns The ID of the newly created document.
 */
export async function saveSerpDataDoc(
    dataToSave: Omit<FirebaseSerpDataDoc, 'id' | 'createdAt' | 'updatedAt'>
): Promise<string> {
    ...
    try {
        ...
        const docRef = await db.collection(COLLECTIONS.SERP_DATA).add(docData);
        ...
        throw new Error(`Failed to save SERP data document: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function findSerpDataDoc(
    ...
    region: string,
    language: string
): Promise<(FirebaseSerpDataDoc & { id: string }) | null> {
    if (!db) throw new Error('Database not initialized');
    console.log(`[DB-SERP-DATA] Searching for existing doc: Q=${query}, R=${region}, L=${language}`);
    try {
        const snapshot = await db.collection(COLLECTIONS.SERP_DATA)
            .where('query', '==', query)
            .where('region', '==', region)
        ...
        const data = doc.data() as FirebaseSerpDataDoc;
        ...
        throw new Error(`Failed to find SERP data document: ${error instanceof Error ? error.message : String(error)}`);
    }
}
 ...
 * @param fieldName The name of the field to update (must be a key of FirebaseSerpDataDoc).
 * @param value The new value for the field.
 * @returns True if update was successful.
 */
export async function updateSerpDataField<K extends keyof FirebaseSerpDataDoc>(
    docId: string,
    fieldName: K,
    value: FirebaseSerpDataDoc[K]
): Promise<boolean> {
    if (!db) throw new Error('Database not initialized');
    if (!docId) throw new Error('Document ID is required for update');
    console.log(`[DB-SERP-DATA] Updating field '${fieldName}' for doc: ${docId}`);
    try {
        await db.collection(COLLECTIONS.SERP_DATA).doc(docId).update({
            [fieldName]: value,
            updatedAt: Timestamp.now()
        });
        console.log(`[DB-SERP-DATA] Successfully updated field '${fieldName}' for doc: ${docId}`);
        return true;
    } catch (error) {
        console.error(`[DB-SERP-DATA] Error updating field '${fieldName}' for doc ${docId}:`, error);
        throw new Error(`Failed to update field '${fieldName}' for doc ${docId}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function getSerpDataDocById(
    ...
    try {
        const docSnap = await db.collection(COLLECTIONS.SERP_DATA).doc(docId).get();
        if (!docSnap.exists) {
            console.log(`[DB-SERP-DATA] Document not found: ${docId}`);
            return null;
        }
        const data = docSnap.data() as FirebaseSerpDataDoc;
        console.log(`[DB-SERP-DATA] Successfully retrieved document: ${docId}`);
        return { ...data, id: docSnap.id };
    } catch (error) {
        console.error(`[DB-SERP-DATA] Error getting document by ID ${docId}:`, error);
        throw new Error(`Failed to get document by ID ${docId}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function deleteSerpDataDocById(docId: string): Promise<boolean> {
    ...
    try {
        await db.collection(COLLECTIONS.SERP_DATA).doc(docId).delete();
        ...
        throw new Error(`Failed to delete document ${docId}: ${error instanceof Error ? error.message : String(error)}`);
    }
}