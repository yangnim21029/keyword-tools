import { htmlAnalysisResultSchema } from '@/lib/schemas/serp.schema';
...
export async function updateSerpResultWithHtmlAnalysis(documentId: string, keyword: string, url: string, htmlAnalysis: any) {
  ...
  try {
    ...
    // 更新历史记录文档，使用关键词和URL作为标识
    const docRef = db.collection(COLLECTIONS.KEYWORD_RESEARCH_HISTORY).doc(documentId);
