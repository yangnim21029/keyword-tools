...
export function SettingsDialog() {
  ...
  // Destructure state
  const { filterZeroVolume, maxResults, region, language, regions, languages, useAlphabet, useSymbols } = settingsState

  // Destructure actions
  const { setFilterZeroVolume, setMaxResults, setRegion, setLanguage, setUseAlphabet, setUseSymbols } = settingsActions
          ...
          <TabsContent value="general" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="settings-region" className="text-sm font-medium flex items-center">
                  地區
              ...
              <div className="space-y-2">
                <Label htmlFor="settings-language" className="text-sm font-medium flex items-center">
                  語言
                  <Badge variant="outline" className="ml-2 text-xs">
                    {language}
                  </Badge>
          ...
          <TabsContent value="search" className="space-y-4">
            <div className="space-y-4">
              <div className="flex items-start space-x-2">
                <Checkbox id="settingsUseAlphabet" checked={useAlphabet} onCheckedChange={setUseAlphabet} />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="settingsUseAlphabet" className="text-sm font-medium cursor-pointer">
                    包含字母 (用於關鍵詞建議)
                  </Label>
                  <p className="text-xs text-gray-500 dark:text-gray-400">自動添加 A-Z 字母到關鍵詞建議中</p>
                </div>
              </div>

              <div className="flex items-start space-x-2">
                <Checkbox id="settingsUseSymbols" checked={useSymbols} onCheckedChange={setUseSymbols} />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="settingsUseSymbols" className="text-sm font-medium cursor-pointer">
                    包含符號 (用於關鍵詞建議)
                  </Label>
                  <p className="text-xs text-gray-500 dark:text-gray-400">自動添加常用符號到關鍵詞建議中</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-2">
                ...
                />
                <div className="grid gap-1.5 leading-none">
                  <Label htmlFor="filter-zero" className="text-sm font-medium cursor-pointer">
                    過濾零搜索量關鍵詞
                  </Label>
                  <p className="text-xs text-gray-500 dark:text-gray-400">自動過濾沒有搜索量的關鍵詞</p>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <div className="flex justify-between items-center">
                  <Label htmlFor="max-results" className="text-sm font-medium">
                    最大結果數
                    <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">({maxResults} 個關鍵詞)</span>
                  </Label>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  />
                  <span className="text-sm font-medium w-8 text-center">{maxResults}</span>
                </div>
