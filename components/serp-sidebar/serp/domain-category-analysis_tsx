...
const getDomainCategory = (domain: string): string => {
  ...
  for (const d of ownMediaDomains) {
    if (baseDomain === d || baseDomain.endsWith('.' + d)) {
      // More specific match for own media
      return '自家媒體';
    }
  }
...
const defaultDomainCompetition: Record<
  string,
...
// 分析搜尋結果競爭度
const analyzeSerpCompetition = (
  results: SerpResultItem[],
  competitionData: Record<string, { level: string; category: string }>
): {
  categorizedDomains: Array<{
    position: number;
    domain: string;
    category: string;
  }>;
  ...
  const ownMediaCount = categorizedResults.filter(
    r => r.category === '自家媒體'
  ).length;

  // Calculate available positions (Unknown category) among the categorized top results
  const availablePositions = categorizedResults.filter(
  ...
  return {
    ...
    ownMediaCount,
    competitionLevel:
      competitionStandards[competitionKey as keyof typeof competitionStandards]
        .label,
    competitionClass:
      competitionStandards[competitionKey as keyof typeof competitionStandards]
        .className
